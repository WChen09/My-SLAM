...
void Tracking::TrackLastObjects(std::vector<int> &_assignment, const float nMatchesRatio, std::vector<DetectedObject> &lastObjectProInCurrent)
{
			...
			iLastObject < lastObjectProInCurrent.size(); iLastObject++)
	{
        ...
        std::cout << "Object " << mLastFrame.mvObjectId.at(iLastObject) << " MPs matching: " << nMatchRatio << std::endl;
        // < means Lost or have left current view
...
{
	// update current frame object ID
    ...
    for(size_t i = 0; i < ObjectMPs.size(); i++)
    {
        ...
        if(iObjectMPs.size() == 0)
        {
            continue;
        }
        // PCL filter remove
        std::vector<MapPoint*> iObjectMPs_filter;
        PCLStatisticalFilter(iObjectMPs, iObjectMPs_filter, 10, 0.5);
	...
	// update Local object Map
	updateObjectMapPoints();
	mCurrentFrame.reOrgnizeFeature();

    //show 2d bounding box in map
    if(ObjectBoxCornerLocation.size()!=0)
    {
    	//
        ...
        //convert x3Dc to 3DPw
        std::vector<std::vector<cv::Point3f>> vv3DPw;
        vv3DPw.resize(ObjectBoxCornerLocation.size());
//    	std::cout << std::endl << std::endl;
        for(size_t i = 0; i < vv3DPw.size(); i++)
        {
            vv3DPw.at(i).resize(4);
            std::vector<cv::Mat> vx3Dc = ObjectBoxCornerLocation.at(i);
            for(size_t j = 0; j < vx3Dc.size(); j++)
            {

            	cv::Mat x3Dc = vx3Dc.at(j);
            	cv::Mat x3Dw = Rwc*x3Dc + twc;
            	cv::Point3f x3DPw(x3Dw.at<float>(0), x3Dw.at<float>(1), x3Dw.at<float>(2));
            	vv3DPw.at(i).at(j) = x3DPw;
//            	std::cout << x3DPw;
            }
        }
        mpMap->SetBBoxInMap(vv3DPw, ObjectMPsFilter);
    }

	// using Epipolar line constrain
    CheckObjectMoving(_assignment);
//    CheckObjectMovingByCameraPose();

	// first, add (tracking done) Object to Map, compute pose and filtering its MapPoints
	for(size_t i = 0; i < mLastFrame.mvObjectId.size(); i++)
	{
        if(mLastFrame.mvObjectId.size() == 0)
            break;
		if(mvObjectStatus.at(mLastFrame.mvObjectId.at(i)) == false)// mean track done
		{
			int DoneObjectId = mLastFrame.mvObjectId.at(i);
			std::vector<MapPoint*> vDoneObjectMPs = mvvObjectMps.at(DoneObjectId);


			//Condition not to add in Object Map
            bool a1 = vDoneObjectMPs.size() < 10;
			bool a2 = mvObjectObserveTimes.at(DoneObjectId) < 5;
			bool a3 = mvObjectMoving.at(DoneObjectId);

            bool needRemove = a1||a2||a3;

            if(needRemove)
            {
                mvObjectIsBad.at(DoneObjectId) = true;
                std::cout << "Skip the Object " << DoneObjectId
                        << " (" << a1 << ", " << a2 << ", " << a3 << ")" << std::endl;
                for(size_t iMp = 0; iMp < vDoneObjectMPs.size(); iMp++)
                {
                    vDoneObjectMPs.at(iMp)->setClassId(-1);
//                    vDoneObjectMPs.at(iMp)->setObjectId(-1);
                    vDoneObjectMPs.at(iMp)->SetBadFlag();
                }
                continue;
            }

            std::vector<MapPoint*> vObjectMPsFilter1;
            PCLStatisticalFilter(vDoneObjectMPs, vObjectMPsFilter1, (int)(vDoneObjectMPs.size()/1), 0.01);
//            std::cout << "add object " << DoneObjectId << " " << vDoneObjectMPs.size() << " -> " << vObjectMPsFilter.size() <<  std::endl;
            ...
            PCLStatisticalFilter(vDoneObjectMPs, vObjectMPsFilter2, (int)(vDoneObjectMPs.size()/2), 0.01);
//            std::cout << "add object " << DoneObjectId << " " << vDoneObjectMPs.size() << " -> " << vObjectMPsFilter2.size() <<  std::endl;
            cv::Point3f PoseStd2;
...
//            std::cout << "add object " << DoneObjectId << " "
//                      << vDoneObjectMPs.size() << " -> " << vObjectMPsFilter.size()
//                      << " std " << PoseStd <<  std::endl;
			std::set<MapPoint*> sObjectMPsFilter(vObjectMPsFilter.begin(), vObjectMPsFilter.end());
			//remove MapPoint outlier label;
            for(size_t j = 0; j < vDoneObjectMPs.size(); j++)
            {
                if(sObjectMPsFilter.count(vDoneObjectMPs.at(j)))
                {
                    continue;
                }
                else
                {
                    vDoneObjectMPs.at(j)->setClassId(-1);
                    vDoneObjectMPs.at(j)->setObjectId(-1);
                }
            }

            std::vector<float> range = ComputeRange(vObjectMPsFilter);
//			mvObjectStatus.at(DoneObjectId) = false;
			mpMap->AddObjectMapPoints(vObjectMPsFilter);
            mpMap->AddObjectPose(ObjectPose, DoneObjectId, range);
		}
	}
	//std::cout << std::endl;
}
...
void Tracking::FirstTrack()
{
	...
	for(size_t i = 0; i < ObjectMPs.size(); i++)
	{
		...
		// PCL filter remove
		std::vector<MapPoint*> iObjectMPs_filter;
		PCLStatisticalFilter(iObjectMPs, iObjectMPs_filter, 5, 1);
		//        ObjectsPose.at(i) = ComputeObjectPose(iObjectMPs_filter);
	...
	//    std::vector<std::vector<cv::KeyPoint>> labeledKps = mCurrentFrame.mvkpsInObject;
	//    for(size_t i = 0; i < labeledKps.size(); i++)
	//    {
	//
	//    	std::cout  << i << " labeled KPs " << labeledKps.at(i).size() << std::endl;
	//    }

}

void Tracking::PCLStatisticalFilter(std::vector<MapPoint *> &InMps, std::vector<MapPoint *> &OutMps, const int& nNeighbors, const double& stdDevMult)
{
...
cv::Point3f Tracking::ComputeObjectPose(std::vector<MapPoint *> &MPs, cv::Point3f & PoseStd)
{
	if(MPs.size() == 0)
	{
...
float Tracking::meadianBlurMeanStd(std::vector<float> &P, float & std)
{
...
void Tracking::updateObjectMapPoints()
{
	...
	{
		// update Object MapPoints bag
		for(size_t iC = 0; iC < mCurrentFrame.mvObjectMPs.size(); iC++)
		{
			std::cout << msObjectId.count(mCurrentFrame.mvObjectId.at(iC)) << std::endl;

			if(msObjectId.count(mCurrentFrame.mvObjectId.at(iC)) == 1)
			{
			...
			else
			{
				...
				mvObjectStatus.push_back(true);
				mvObjectMoving.push_back(false);
				mvObjectIsBad.push_back(false);
			}
		}

        if(mLastFrame.mvObjectMPs.size()==0)
			return;

		// update Object traking status
		std::vector<int> LastObjectId = mLastFrame.mvObjectId;
		std::vector<int> vCurrentObjectId = mCurrentFrame.mvObjectId;
		std::set<int> sCurrentObjectId(vCurrentObjectId.begin(), vCurrentObjectId.end());
		for(size_t iLast = 0; iLast < LastObjectId.size(); iLast++)
		{
			if(sCurrentObjectId.count(LastObjectId.at(iLast)))
				continue;
			else
			{
				mvObjectStatus.at(LastObjectId.at(iLast)) = false;
			}
		}
	}
}

void Tracking::CheckObjectMoving(std::vector<int>& _assignment)
{
	std::vector<std::vector<std::pair<cv::KeyPoint, cv::KeyPoint>>> vvObjectMatchKps;
	std::vector<int> vObjectMatchIdx;
	vvObjectMatchKps.reserve(mLastFrame.mvObjects.size());
	vObjectMatchIdx.reserve(mLastFrame.mvObjects.size());

	std::vector<std::pair<cv::KeyPoint, cv::KeyPoint>> ObjectMatchKps;
	ObjectMatchKps.reserve(mCurrentFrame.N);
	for(size_t iLast = 0; iLast < _assignment.size(); iLast++)
	{
		if(_assignment.at(iLast) == -1)
			continue;

		std::vector<cv::KeyPoint> LastObjectKPs = mLastFrame.mvkpsInObject.at(iLast);
		std::vector<cv::KeyPoint> CurrentObjectKPs = mCurrentFrame.mvkpsInObject.at(_assignment.at(iLast));
		cv::Mat LastDescriptor = mLastFrame.mvdescriptorsInObject.at(iLast);
		cv::Mat CurrentDescriptor = mCurrentFrame.mvdescriptorsInObject.at(_assignment.at(iLast));

		cv::Mat F12 = ComputeF12(mLastFrame, mCurrentFrame);

		ORBmatcher matcher(0.9,true);
		vector<pair<size_t, size_t> > matchedIdx;
		int nmatches = matcher.SearchForObjectMoving(mLastFrame, mCurrentFrame, LastObjectKPs, LastDescriptor,
				CurrentObjectKPs, CurrentDescriptor, F12, matchedIdx);

		float matchRatio = ((float) nmatches)/(0.5*(float)LastObjectKPs.size()+0.5*(float)CurrentObjectKPs.size());

		std::cout << "moving check match " << nmatches << " ratio:" << matchRatio << std::endl;
		std::vector<std::pair<cv::KeyPoint, cv::KeyPoint>> vObjectMatchKps;
		vObjectMatchKps.resize(nmatches);
		for(size_t imatch = 0; imatch < matchedIdx.size(); imatch++)
		{
			auto matchPair = std::make_pair(LastObjectKPs.at(matchedIdx.at(imatch).first), CurrentObjectKPs.at(matchedIdx.at(imatch).second));
			ObjectMatchKps.push_back(matchPair);
			vObjectMatchKps.at(imatch) = matchPair;
		}

		vvObjectMatchKps.push_back(vObjectMatchKps);
		vObjectMatchIdx.push_back(iLast);
        if(nmatches < 10)//not so good
			mvObjectMoving.at(mLastFrame.mvObjectId.at(iLast)) = true;
	}
	mvObjectMatchKps = ObjectMatchKps;


	// have known the object average depth, then the length of Feature Flow vector should be equal the K*t/depthAvg

	const cv::Mat R1w = mLastFrame.mTcw.rowRange(0,3).colRange(0,3); //3x3
	const cv::Mat t1w = mLastFrame.mTcw.rowRange(0,3).col(3); //3x1

	const cv::Mat R2w = mCurrentFrame.mTcw.rowRange(0,3).colRange(0,3); //3x3
	const cv::Mat t2w = mCurrentFrame.mTcw.rowRange(0,3).col(3); //3x1
	cv::Mat R12 = R1w*R2w.t();
	cv::Mat t12 = -R1w*R2w.t()*t2w+t1w;
//	for(size_t i = 0; i < vvObjectMatchKps.size(); i++)
//	{
//        float ObjectDepth = mCurrentFrame.mvObjectDepth.at(_assignment.at(vObjectMatchIdx.at(i)));
//		cv::Mat avgObjectFlow = mK*t12/ObjectDepth;
//		std::cout << "avg object flow: " << avgObjectFlow << std::endl;
//		std::vector<std::pair<cv::KeyPoint, cv::KeyPoint>> CurrentObjectMatchKps = vvObjectMatchKps.at(i);
//		for(size_t j = 0; j < CurrentObjectMatchKps.size(); j++)
//		{
//			cv::Point2f KpsVector = - CurrentObjectMatchKps.at(j).first.pt + CurrentObjectMatchKps.at(j).second.pt;
//			std::cout << "KpsVector: " << KpsVector.x << " " << KpsVector.y << std::endl;
//		}

//	}


    // Flow Vector Bound (FVB) constriant
    //1. compute zmax and zmin in current frame
    int belta = 2;
...
//    std::cout << "z" << std::endl;
    for(size_t iMp = 0; iMp < mCurrentFrame.mvpMapPoints.size(); iMp++)
    {
        ...
        if(pMp)
        {
            cv::Mat X3Dw = pMp->GetWorldPos();
            cv::Mat X3Dc = Rcw * X3Dw + tcw;
            float z = X3Dc.at<float>(2);
            dValue.push_back(cv::norm(mK*t12/z));
//            std::cout << cv::norm(mK*t12/z) << " ";
        }
    }
    auto max_ = std::max_element(std::begin(dValue), std::end(dValue));
    auto min_ = std::min_element(dValue.begin(), dValue.end());
    float dmax = *max_;
    float dmin = *min_;
    float dmean = (dmax + dmin)/2;
    float drange = (dmax - dmin)/2;

//    std::cout << std::endl;
    std::vector<float> ObjectPro(vvObjectMatchKps.size(), 0);
    for(size_t i = 0; i < vvObjectMatchKps.size(); i++)
    {
    	std::vector<std::pair<cv::KeyPoint, cv::KeyPoint>> CurrentObjectMatchKps = vvObjectMatchKps.at(i);
    	float proSum = 0;
    	...
    	{
    		cv::Point2f KpsVector = - CurrentObjectMatchKps.at(j).first.pt + CurrentObjectMatchKps.at(j).second.pt;
    		float d = sqrt(KpsVector.x*KpsVector.x + KpsVector.y*KpsVector.y);
    		float pro = 1.0/(1 + pow(((d-dmean)/drange), 2*belta));
//            std::cout << d << " " << pro << std::endl;
    		proSum += pro;
    	}

    	proSum = proSum/(float)CurrentObjectMatchKps.size();
    	ObjectPro.at(i) = proSum;
    	std::cout << "ObejctPro: " << proSum << std::endl;
    }

}

void Tracking::CheckObjectMovingByCameraPose()
{
    for(size_t i = 0; i < mCurrentFrame.mvObjectMPs.size(); i++)
    {
        if(mCurrentFrame.mvObjectMPs.at(i).size() < 4)
            continue;

        Optimizer::PoseOptimizationCheckMoving(&mCurrentFrame,
                                               mCurrentFrame.mvObjectMPs.at(i),
                                               mCurrentFrame.mvObjectMPsKps.at(i));
...
cv::Mat Tracking::ComputeF12(Frame& F1, Frame& F2)
{
	const cv::Mat R1w = F1.mTcw.rowRange(0,3).colRange(0,3); //3x3
	const cv::Mat t1w = F1.mTcw.rowRange(0,3).col(3); //3x1

	const cv::Mat R2w = F2.mTcw.rowRange(0,3).colRange(0,3); //3x3
	const cv::Mat t2w = F2.mTcw.rowRange(0,3).col(3); //3x1
	cv::Mat R12 = R1w*R2w.t();
	cv::Mat t12 = -R1w*R2w.t()*t2w+t1w;
...
std::vector<float> Tracking::ComputeRange(std::vector<MapPoint*> & Mps)
{
