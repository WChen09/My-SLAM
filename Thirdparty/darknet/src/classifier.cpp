...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    listc *options = read_data_cfg(datacfg);

    char *backup_directory = option_find_str(options, "backup", "/backup/");
    char *label_listc = option_find_str(options, "labels", "data/labels.list");
    char *train_listc = option_find_str(options, "train", "data/train.list");
    int classes = option_find_int(options, "classes", 2);

    char **labels = get_labels(label_listc);
    listc *plistc = get_paths(train_listc);
    char **paths = (char **)list_to_array(plistc);
    printf("%d\n", plistc->size);
    int N = plistc->size;
    clock_t time;
    ...
    free_ptrs((void**)paths, plistc->size);
    free_list(plistc);
    free(base);
}
...
/*
   ...
   listc *plistc = get_paths(train_list);
   char **paths = (char **)list_to_array(plistc);
   printf("%d\n", plistc->size);
   int N = plistc->size;
   clock_t time;
...
void validate_classifier_crop(char *datacfg, char *filename, char *weightfile)
{
    ...
    char *label_listc = option_find_str(options, "labels", "data/labels.list");
    char *valid_listc = option_find_str(options, "valid", "data/train.list");
    int classes = option_find_int(options, "classes", 2);
    int topk = option_find_int(options, "top", 1);

    char **labels = get_labels(label_listc);
    listc *plistc = get_paths(valid_listc);
...
void validate_classifier_single(char *datacfg, char *filename, char *weightfile)
{
    ...
    char *label_listc = option_find_str(options, "labels", "data/labels.list");
    char *leaf_listc = option_find_str(options, "leaves", 0);
    if(leaf_listc) change_leaves(net.hierarchy, leaf_listc);
    char *valid_listc = option_find_str(options, "valid", "data/train.list");
    int classes = option_find_int(options, "classes", 2);
    int topk = option_find_int(options, "top", 1);
...
void try_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int layer_num)
{
    ...
    char *name_listc = option_find_str(options, "names", 0);
    if(!name_listc) name_listc = option_find_str(options, "labels", "data/labels.list");
    int top = option_find_int(options, "top", 1);

    int i = 0;
    char **names = get_labels(name_listc);
    clock_t time;
...
void label_classifier(char *datacfg, char *filename, char *weightfile)
{
    ...
    char *label_listc = option_find_str(options, "names", "data/labels.list");
    char *test_listc = option_find_str(options, "test", "data/train.list");
    int classes = option_find_int(options, "classes", 2);

    char **labels = get_labels(label_listc);
    listc *plistc = get_paths(test_listc);
...
void test_classifier(char *datacfg, char *cfgfile, char *weightfile, int target_layer)
{
    ...
    char *test_listc = option_find_str(options, "test", "data/test.list");
    int classes = option_find_int(options, "classes", 2);
...
void run_classifier(int argc, char **argv)
{
    ...
    char *gpu_listc = find_char_arg(argc, argv, "-gpus", 0);
    int *gpus = 0;
    int gpu = 0;
    int ngpus = 0;
    if(gpu_listc){
        printf("%s\n", gpu_listc);
        int len = strlen(gpu_listc);
        ngpus = 1;
        int i;
        for(i = 0; i < len; ++i){
            if (gpu_listc[i] == ',') ++ngpus;
        }
        gpus = (int*)calloc(ngpus, sizeof(int));
        for(i = 0; i < ngpus; ++i){
            gpus[i] = atoi(gpu_listc);
            gpu_listc = strchr(gpu_listc, ',')+1;
        }
    } else {
        gpu = gpu_index;
        gpus = &gpu;
