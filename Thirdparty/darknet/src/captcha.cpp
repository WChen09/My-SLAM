...
void train_captcha(char *cfgfile, char *weightfile)
{
    ...
    if (solved){
        plistc = get_paths("/data/captcha/reimgs.solved.list");
    }else{
        plistc = get_paths("/data/captcha/reimgs.raw.list");
    }
...
void valid_captcha(char *cfgfile, char *weightfile, char *filename)
{
    ...
    listc *plistc = get_paths("/data/captcha/reimgs.fg.list");
    char **paths = (char **)list_to_array(plistc);
    int N = plistc->size;
...
/*
   ...
   int imgs = 1024;
   int i = net.seen/imgs;
   listc *plistc = get_paths("/data/captcha/train.auto5");
   char **paths = (char **)list_to_array(plistc);
   printf("%d\n", plistc->size);
   clock_t time;
   while(1){
   ++i;
   time=clock();
   data train = load_data_captcha(paths, imgs, plistc->size, 10, 200, 60);
...
int imgs = 1024;
int i = net.seen/imgs;
listc *plistc = get_paths("/data/captcha/encode.list");
char **paths = (char **)list_to_array(plistc);
printf("%d\n", plistc->size);
clock_t time;
while(1){
    ++i;
    time=clock();
    data train = load_data_captcha_encode(paths, imgs, plistc->size, 300, 57);
...
void validate_captcha(char *cfgfile, char *weightfile)
{
    ...
    int numchars = 37;
    listc *plistc = get_paths("/data/captcha/solved.hard");
    char **paths = (char **)list_to_array(plistc);
    int imgs = plistc->size;
