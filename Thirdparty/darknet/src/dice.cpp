...
void train_dice(char *cfgfile, char *weightfile)
{
    ...
    listc *plistc = get_paths("data/dice/dice.train.list");
    ...
    while(1){
        ++i;
        time=clock();
        data train = load_data_old(paths, imgs, plistc->size, labels, 6, net.w, net.h);
...
void validate_dice(char *filename, char *weightfile)
{
    ...
    char **labels = dice_labels;
    listc *plistc = get_paths("data/dice/dice.val.list");
